...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <head>
        ...
        <title>Número de vocales</title>
        @vite('resources/css/app.css')
    </head>
    <body>
        <section class="container mx-auto bg-slate-200">
            <h1 class="text-5xl text-center font-bold underline">
                Determinar el número de vocales de una cadena
            </h1>
            <form action="{{ route('obtenerVocales') }}" method="post" class=" bg-green-200 p-5 text-2xl grid grid-cols-1 items-center">
                @csrf
            ...
            @isset($cant)
                <section class="bg-violet-200 text-xl text-center">
                    El número de vocales de la cadena <br>"{{ $cad }}"<br> es: | {{ $cant }} |
                </section>
            @endisset
